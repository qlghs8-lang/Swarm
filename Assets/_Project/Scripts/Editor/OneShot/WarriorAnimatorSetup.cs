...
{
    public static class WarriorAnimatorSetup
    {
        ...
        private const string ControllerPath = ControllerDirectory + "/Warrior.controller";
        ...
        [MenuItem("Swarm/Art Test/Setup Warrior Idle-Walk Animator (Both Scenes)")]
        private static void Setup()
        {
            ...
            Debug.Log("Warrior Animator setup complete for Game.unity and TestStage.unity. Enter Play Mode and move the player to check the Idle/Walk loop.");
        }

        private static AnimatorController BuildController()
        {
            var asepriteAssets = AssetDatabase.LoadAllAssetsAtPath(WarriorAsepritePath);
            ...
            if (idleClip == null || walkClip == null)
            {
                ...
                Debug.LogError($"Could not find both 'Idle' and 'Walk' AnimationClip sub-assets in {WarriorAsepritePath}. Found clips: [{foundNames}]. Right-click the file in the Project window and choose Reimport, then run this again.");
                return null;
            }
        ...
        private static void ApplyToScene(string scenePath, AnimatorController controller)
        {
            ...
            {
                var frame0 = AssetDatabase.LoadAllAssetsAtPath(WarriorAsepritePath)
                    .OfType<Sprite>()
