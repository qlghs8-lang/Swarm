    ...
    public class WarriorRageWeapon : LevelableWeapon
    {
        ...
        private void Awake()
        {
            ...
            _smashEffect = SpriteEffectPlayer.Create(
                this, "RageSmashEffect (Temp)", effectMaterial, smashEffectFrames, smashFrameDuration);
        }
        ...
        private void HandleDamageDealt(GameObject target)
        {
            ...
            if (Random.value <= stackChance)
            {
                StackCount++;
                if (_stats != null) _stats.IncreaseAttackPower(attackPowerPerStack);
                if (_health != null) _health.IncreaseMaxHealth(maxHealthPerStack);
            }
